...
//copy of https://github.com/phuccom000/Overgeared/blob/1.21.1/src/main/java/net/stirdrem/overgeared/event/AnvilMinigameEvents.java
...
public class AnvilMinigameEvents {
    ...
    public static String handleHit() {
        ...
        if (arrowPosition >= perfectZoneStart && arrowPosition <= perfectZoneEnd) {
            ...
            triggerPopup(Component.translatable("overtailored.forging.perfect")
                    .withStyle(s -> s.withBold(true).withColor(0xFFD700)));
        } else if (arrowPosition >= goodZoneStart && arrowPosition <= goodZoneEnd) {
            goodHits++;
            lastGood = goodHits;
            triggerPopup(Component.translatable("overtailored.forging.good")
                    .withStyle(s -> s.withBold(true).withColor(0x55FF55)));
        } else {
            missedHits++;
            lastMiss = missedHits;
            triggerPopup(Component.translatable("overtailored.forging.miss")
